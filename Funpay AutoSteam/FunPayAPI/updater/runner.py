...
class Runner:
    ...
    def get_result(self, payload: dict) -> requests.Response:
        ...
        if not self.__is_running:
            return self.account.runner_request(payload)
        ...
        while id_ in self.payload_queue:
            time.sleep(0.1)
            wait_cycles += 1
            if wait_cycles > 100:  # 10s fallback
    ...
    def listen(self, requests_delay: int | float = 6.0,
        ...
        """
        if not self.__is_running:
            loop_thread = threading.Thread(target=self.loop, daemon=True, name="FunPayRunnerLoop")
            loop_thread.start()
            time.sleep(0.05)
